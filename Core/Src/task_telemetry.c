...
#include <telemetry_labels.h>     // Required for string lookup table, Side_t, LabelID_t
...
void StartTask_Telemetry(void * argument) {
	...
	switch(last_reset_cause) {
		case RST_BOR:
			rst_str = "BOR"; // Brown-Out: Normal power cycle or voltage drop
			break;
		case RST_OBL:
			rst_str = "OBL"; // Option Byte Loader: Memory configuration reload
			break;
		case RST_PIN:
			rst_str = "PIN"; // Pin Reset: External NRST pin was pulled low
			break;
		case RST_SFT:
			rst_str = "SFT"; // Software Reset: Code explicitly rebooted the MCU
			break;
		case RST_IWDG:
			rst_str = "WDG"; // Independent Watchdog: Safety timer forced a reboot
			break;
		case RST_WWDG:
			rst_str = "WWD"; // Window Watchdog: Timing violation during refresh
			break;
		case RST_LPWR:
			rst_str = "LPW"; // Low-Power Reset: Illegal deep sleep mode entry
			break;
		case RST_FWR:
			rst_str = "FWR"; // Firewall Reset: Unauthorized memory access blocked
			break;
		default:
			rst_str = "OTH"; // Other/Unknown: Unmapped hardware reset event
			break;
	}
    ...
    while(1) {
    	...
    	/* String to accumulate hardware and software timing errors for this second */
		char err_str[128] = "";
		...
		for(uint8_t ch = 0; ch < MUX_CHANNELS; ch++) {
			...
			for(int m = 0; m < NUM_MUX; m++) {
				...
				if (HAL_ADC_PollForConversion(&hadc2, 1) == HAL_OK) {
					...
					/* Append to the error tracking string if not already present */
					if (strstr(err_str, "INT_ADC_TO") == NULL) {
						strcat(err_str, "INT_ADC_TO ");
					}
				}
			}
		}
		...
		for (int i = 0; i < NUM_INA260; i++) {
			...
			if (pingPongBuf[processIdx].ina_voltage[i].count > 0) {
				...
				//Format the specific I2C address that failed and append it to the error log
				char to_str[16];
				snprintf(to_str, sizeof(to_str), "I2C_0x%02X_TO ", INA_I2C_ADDRS[i]);
				if (strstr(err_str, to_str) == NULL) strcat(err_str, to_str);
			}
			...
			for (int ch = 0; ch < SPI_ADC_CHANNELS; ch++) {
				if (pingPongBuf[processIdx].spi_adc[adc][ch].count > 0) {
					...
					/* SPI Timeout! If channel 0 is missing data, we assume the entire
					   chip failed. We only append the error when ch == 0 to avoid duplicates. */
					if (ch == 0) {
						char to_str[16];
		...
		/* --- 4. FORMAT AND SEND UART (JSON LOG) --- */
		...
		if (daq_is_alive == 0) {
			strcat(err_str, "DAQ_TASK_TO ");
		}

		/* Clean up the error string for JSON output */
		if (strlen(err_str) == 0) {
			strcpy(err_str, "NONE"); /* No errors detected */
		} else {
			err_str[strlen(err_str) - 1] = '\0'; /* Remove the trailing space */
		}

		/* 4a. Format JSON: Start directly with the core telemetry payload (MS & FW) */
		len += snprintf(txBuffer + len, sizeof(txBuffer) - len,
			"{\"Uptime\":%lu,\"Rst\":\"%s\",\"Error\":\"%s\",\"HW_ver\":\"0x%02X\",\"SW_ver\":\"%d.%d.%d\",\"UID\":\"0x%08lX\",\"Side\":\"%s\","
		...
		/* 4b. Format JSON: FP, MD, AUX1, CHR */
		len += snprintf(txBuffer + len, sizeof(txBuffer) - len,
			"\"%s\":{\"%s\":{\"Min\":%.3f,\"Max\":%.3f,\"Avg\":%.3f},\"%s\":{\"Min\":%.3f,\"Max\":%.3f,\"Avg\":%.3f},\"%s\":{\"Min\":%.3f,\"Max\":%.3f,\"Avg\":%.3f},\"%s\":{\"Min\":%.3f,\"Max\":%.3f,\"Avg\":%.3f}},"
			"\"%s\":{\"V\":{\"Min\":%.3f,\"Max\":%.3f,\"Avg\":%.3f},\"I\":{\"Min\":%.3f,\"Max\":%.3f,\"Avg\":%.3f}},"
			"\"%s\":{\"V\":{\"Min\":%.3f,\"Max\":%.3f,\"Avg\":%.3f},\"I\":{\"Min\":%.3f,\"Max\":%.3f,\"Avg\":%.3f}},"
			"\"%s\":{\"V\":{\"Min\":%.3f,\"Max\":%.3f,\"Avg\":%.3f},\"I\":{\"Min\":%.3f,\"Max\":%.3f,\"Avg\":%.3f},\"Src\":\"%s\"},",
			...
			GET_SRC_STR(muxData.fields.chr1_valid_n, muxData.fields.chr2_valid_n) // Evaluates to "UPS1" / "UPS2", "NONE", or "UNKNOWN"
		);

		/* 4c. Format JSON: AS, MC, FAN, PSEC, LPRC */
		len += snprintf(txBuffer + len, sizeof(txBuffer) - len,
			"\"%s\":{\"V\":{\"Min\":%.3f,\"Max\":%.3f,\"Avg\":%.3f},\"I\":{\"Min\":%.3f,\"Max\":%.3f,\"Avg\":%.3f},\"Src\":\"%s\"},"
			"\"%s\":{\"V\":{\"Min\":%.3f,\"Max\":%.3f,\"Avg\":%.3f},\"I\":{\"Min\":%.3f,\"Max\":%.3f,\"Avg\":%.3f}},"
			"\"%s\":{\"V\":{\"Min\":%.3f,\"Max\":%.3f,\"Avg\":%.3f},\"I\":{\"Min\":%.3f,\"Max\":%.3f,\"Avg\":%.3f}},"
		...
		/* 4d. Format JSON: Internal Power Architecture */
		len += snprintf(txBuffer + len, sizeof(txBuffer) - len,
			"\"%s\":{\"V\":{\"Min\":%.3f,\"Max\":%.3f,\"Avg\":%.3f}},"
			"\"%s\":{\"V\":{\"Min\":%.3f,\"Max\":%.3f,\"Avg\":%.3f}},"
			"\"28V_INT\":{\"V\":{\"Min\":%.3f,\"Max\":%.3f,\"Avg\":%.3f},\"Src\":\"%s\"},"
			"\"7.5V\":{\"V\":{\"Min\":%.3f,\"Max\":%.3f,\"Avg\":%.3f}},"
			"\"5V\":{\"V\":{\"Min\":%.3f,\"Max\":%.3f,\"Avg\":%.3f}},"
			"\"3.3V\":{\"V\":{\"Min\":%.3f,\"Max\":%.3f,\"Avg\":%.3f}},",
		...
		/* 4e. Format JSON: Remaining internal rails and PCP */
		len += snprintf(txBuffer + len, sizeof(txBuffer) - len,
			"\"5V_MCU\":{\"V\":{\"Min\":%.3f,\"Max\":%.3f,\"Avg\":%.3f}},"
			"\"3.3V_MCU\":{\"V\":{\"Min\":%.3f,\"Max\":%.3f,\"Avg\":%.3f}},"
